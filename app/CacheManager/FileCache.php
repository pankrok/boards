...
class FileCache{
  ...
  public function __construct($config = null, $logger) {
	...
	$this->logger = $logger;  
	$this->logger->info('File Cache Init');
  ...
  public function isCached($key) {
    ...
    }else{$this->logger->warn("!!! isCached not found: $key !!!");}
  }
  /**
  ...
  public function store($key, $data, $expiration = 0) {
    ...
    $cacheData = base64_encode(json_encode($dataArray));
  ...
  public function retrieve($key, $timestamp = false) {
	...
	{
		$this->logger->warn("!!! retrieve not found: $key !!!");
		return null;
	}		
  ...
  public function retrieveAll($meta = false) {
	$this->logger->info("retrieveAll $key");
    if ($meta === false) {
      $results = array();
      $cachedData = $this->_loadCache();
      if ($cachedData) {
  ...
  public function erase($key) {
    ...
    if (true === is_array($cacheData)) {
      if (true === isset($cacheData[$key])) {
        unset($cacheData[$key]);
        $cacheData = base64_encode(json_encode($cacheData));
        file_put_contents($this->getCacheDir(), $cacheData);
      } else {
  ...
  private function _loadCache() {
    if (true === file_exists($this->getCacheDir())) {
      ...
      return json_decode(base64_decode($file), true);
    } else {
      return false;
    }
  }
  ...
  public function setCache($name) {
	$this->logger->info("*** setCache $name");
