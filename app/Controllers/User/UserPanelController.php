...
class UserPanelController extends Controller
{
	...
	public function postProfilePicture($request, $response)
	{
			...
			if($user->avatar)
			{
				...
				@unlink(MAIN_DIR.'/public/upload/'.$delete->original);
				@unlink(MAIN_DIR.'/cache/img/'.$delete->_38);
				@unlink(MAIN_DIR.'/cache/img/'.$delete->_85);
				@unlink(MAIN_DIR.'/cache/img/'.$delete->_150);
				$user->avatar = null;
				$user->save();
				$delete->delete();
				$this->cache->eraseAll();
			...
			if ($uploadedFile->getError() === UPLOAD_ERR_OK) {
				$filename = self::moveUploadedFile(MAIN_DIR.'/public/upload/', $uploadedFile); 	
				
				$cache = $this->resizer->resize($filename);		
				...
				$this->cache->setCache('userdata');
				if($this->cache->isCached($user->id))  $this->cache->erase($user->id);
