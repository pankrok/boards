    ...
    public function ShowList($data)
    {
			...
			$servers =[ 
					[
						'id' => 'DD2',
						'type' => 'cs',
						'connect_host' => '37.233.99.56:27015',
					],
					[
						'id' => 'MIN',
						'type' => 'cs',
						'connect_host' => '37.233.99.56:27017',
					]
				];
			...
			if($results["MIN"]["info"]['online']){
			  $address 	= 	($results["MIN"]["info"]['connect_addr']) . ':' . ($results["MIN"]["info"]['connect_port']);
			  $online 	= 	($results["MIN"]["info"]['online']) ? '<span class="fc-green"><i class="fas fa-circle-notch fa-spin"></i> ONLINE</span>' : '<span class="fc-r"><i class="fas fa-circle-notch "></i> OFFLINE</span>';
			  $name 	=	($results["MIN"]["general"]['hostname']);
			  $map 		=	($results["MIN"]["general"]['map']);
			  $players 	= 	($results["MIN"]["general"]["num_players"]);
			  $max 		=	($results["MIN"]["general"]['max_players']);
			}
			else
			{
			  $address 	= 	($results["MIN"]["info"]['connect_addr']) . ':' . ($results["MIN"]["info"]['connect_port']);
			  $online 	= 	'<span class="fc-r"><i class="fas fa-circle-notch "></i> OFFLINE</span>';
			  $name 	=	'[PL] S89.EU ☆ Mirage-Nuke-Inferno';
			  $map 		=	'-';
			  $players 	= 	0;
			  $max 		=	32;
			  $img		=	'/plugins/serverlist/ico/cs16.png';
