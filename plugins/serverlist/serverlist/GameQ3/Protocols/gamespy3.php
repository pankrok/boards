...
class Gamespy3 extends \GameQ3\Protocols
{
    ...
    protected function _process_all($packets)
    {
        ...
        foreach ($packets as $data) { // Loop thru packets
            ...
            while ($buf->getLength()) { // Loop thru sections
                ...
                while ($buf->getLength()) { // Loop thru fields
                    ...
                    if ($field === "") {
                        if ($section_num == 0) {
                            if ($this->_parse_rules_break($buf)) {
                                break;
                            }
                        } else {
                            break;
        ...
        $res = true;
        if (!empty($fields[1])) {
            $res = $res && $this->_parse_arrays('players', $fields[1]);
        }
        if (!empty($fields[2])) {
            $res = $res && $this->_parse_arrays('teams', $fields[2]);
        }
