...
use Plugins\ServerList\serverlist\GameQ3\Protocols as Protocols;
use Plugins\ServerList\serverlist\GameQ3\Buffer;

class Source extends Protocols
{
    ...
    protected function _process_challenge($packets)
    {
        ...
        if ($this->isRequested('settings')) {
            $this->queue('rules', 'udp', sprintf($this->packets['rules'], $chal));
        }
        if ($this->isRequested('players')) {
            $this->queue('players', 'udp', sprintf($this->packets['players'], $chal));
        }
    }
    ...
    protected function _parse_rules(&$packet)
    {
        ...
        while ($buf->getLength()) {
            ...
            switch ($key) {
                case 'tf_gamemode_arena':	if ($val == 1) {
                    $m = 'arena';
                } break;
                case 'tf_gamemode_cp':		if ($val == 1) {
                    $m = 'cp';
                } break;
                case 'tf_gamemode_ctf':		if ($val == 1) {
                    $m = 'ctf';
                } break;
                case 'tf_gamemode_mvm':		if ($val == 1) {
                    $m = 'mvm';
                } break;
                case 'tf_gamemode_payload':	if ($val == 1) {
                    $m = 'payload';
                } break;
                case 'tf_gamemode_sd':		if ($val == 1) {
                    $m = 'sd';
                } break;
            }
    ...
    protected function _process_details($packets)
    {
        ...
        } elseif ($type === "\x49" || $type === "\x44") { // 0x44? wtf?
