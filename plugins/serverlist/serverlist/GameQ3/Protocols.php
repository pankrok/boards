...
abstract class Protocols
{
    ...
    public function __construct($server_info, $log)
    {
        ...
        if ($this->network) {
            ...
            } elseif (!$this->connect_addr && $this->query_addr) {
        ...
        if (!isset($this->unst['teams'])) {
            $this->unst['teams'] = true;
        } // Unset teams by default
    ...
    protected function fixPorts($query_port, $connect_port)
    {
        ...
        if (!is_int($this->query_port)) {
            $this->query_port = false;
        }
        if (!is_int($this->connect_port)) {
            $this->connect_port = false;
        }
        ...
        } elseif ($this->ports_type === self::PT_DIFFERENT_COMPUTABLE) {
        ...
        } elseif ($this->ports_type === self::PT_DIFFERENT_NONCOMPUTABLE_FIXED) {
            if ($q_set) {
        ...
        } elseif ($this->ports_type === self::PT_DIFFERENT_NONCOMPUTABLE_VARIABLE) {
            if ($q_set) {
                if ($q_default !== $query_port) {
                    $this->query_port = $query_port;
                    $this->connect_port = false; // This value should be filled by protocols when possible
