...
if ($_POST) {
    ...
    if ($status == $_POST['host'] . ' via TCP/IP') {
        createDB();
        $next = true;
        echo '<div class="col-12"><div class="message blue">DB created! <span class="message-close" aria-hidden="true">&times;</span></div></div>';
    } else {
        echo '<div class="message red">';
        var_dump($status);
        echo' <span class="message-close" aria-hidden="true">&times;</span></div>';
    }
}
...
<div class="col-12 text-justify p-4">
	...
	<form method="post" class="mx-auto" style="width: 200px;" action="?step=1">
	  host:<br>
	  <input type="text" name="host" value="<?php if (isset($_POST['host'])) {
    echo $_POST['host'];
} ?>">
	  <br>
	  database:<br>
    ...
    echo $_POST['password'];
	  	  ...
	  	  <br>
	  prefix:<br>
	  <input type="text" name="prefix" value="<?php if (isset($_POST['prefix'])) {
    echo $_POST['prefix'];
} ?>">
	  <?php if (!$next) {
    echo'
